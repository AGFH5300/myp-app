...
export async function GroupedPaperReader({ paperId, backHref, adminPreview = false, includeUnpublishedQuestions = false }: GroupedPaperReaderProps) {
  ...
  let questionQuery = supabase
    .from('questions')
    .select('id,question_number,question_order,image_url,question_image_path,markscheme_text,markscheme_image_url,markscheme_image_path,marks,display_with_question_id')
    .eq('paper_id', paperId)
  ...
  const readerQuestions: FullPaperReaderQuestion[] = (await resolvePaperReaderQuestions(supabase, questions ?? []))
