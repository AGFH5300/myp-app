...
export function SearchableSelect({ id, name, label, value, options, placeholder, emptyText, onChange, openSelectId, setOpenSelectId }: { id: string; name?: string; label: string; value: string; options: SearchableSelectOption[]; placeholder: string; emptyText: string; onChange: (value: string) => void; openSelectId: string | null; setOpenSelectId: (id: string | null) => void }) {
      ...
      {open ? (
        <div className="absolute z-30 mt-2 w-full rounded-md border border-[#c3c6ce66] bg-white p-2 shadow-lg">
          ...
          <div role="listbox" aria-labelledby={`${id}-label`} className="max-h-64 overflow-y-auto">
            <button type="button" role="option" aria-selected={!value} onMouseDown={(event) => event.preventDefault()} onClick={() => choose('')} className={`block w-full rounded-sm px-3 py-2 text-left transition ${!value ? 'bg-[#00152a] text-white' : activeIndex === 0 ? 'bg-[#f5f3ee] text-[#00152a]' : 'text-[#00152a] hover:bg-[#f5f3ee]'}`}>All</button>
            {filtered.map((option, index) => (
              <button key={option.value} type="button" role="option" aria-selected={option.value === value} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(option.value)} className={`block w-full rounded-sm px-3 py-2 text-left transition ${option.value === value ? 'bg-[#00152a] text-white' : index + 1 === activeIndex ? 'bg-[#f5f3ee] text-[#00152a]' : 'text-[#00152a] hover:bg-[#f5f3ee]'}`}>
