    ...
    <div>
      ...
      <div className="space-y-4">
        ...
        {attempts?.map((attempt) => {
          ...
          return <div key={attempt.id} className="bg-white border border-[#c3c6ce66] p-6 flex items-center justify-between gap-4"><div><h2 className="font-headline text-xl text-[#00152a]">{paper?.title || 'Practice Attempt'} · Q{question?.question_number}</h2><p className="font-body text-sm text-[#43474d] mt-2">Score: {attempt.score ?? 0}/{attempt.max_score ?? 0} ({percent}%)</p></div><div className="flex gap-3"><Link href={`/dashboard/results/${attempt.id}`} className="tsm-btn-secondary">Result</Link>{paper?.id && <Link href={`/dashboard/practice/${paper.id}`} className="tsm-btn-primary">Practice Again</Link>}</div></div>
        })}
      </div>
